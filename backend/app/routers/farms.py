...
KNN_CONFIDENT_KM = 0.5  # 이 거리 이내 KNN 매칭만 "농지 맞음"으로 신뢰.
...
def _call_farm_card(lon: float, lat: float, crop_code: str, address_sido: str | None, conn):
    ...
    (지번 주소 지오코딩 좌표가 실제 필지 폴리곤 밖에 찍히는 경우가 흔함).
    다만 KNN은 "그냥 제일 가까운 과수원"을 찾는 것일 뿐 그 위치가 농지라는
    보장이 없다 — 검색 좌표가 도심 한복판이어도 같은 시도 안에서 가장 가까운
    과수원이 뭐든 하나는 잡힌다. 그래서 KNN_CONFIDENT_KM(500m) 이내인
    경우만 "이 주소가 그 필지"로 인정하고, 그보다 멀면(과거엔 20km까지도
    허용해 서울 좌표 → 67km 거리 천안시 필지가 조용히 매칭되는 버그가 있었음)
    매칭 실패(None)로 처리해 "필지 못 찾음 → 면적 직접 입력 + 시도 단위
    폴백"(db_loader._load_land)으로 넘긴다 — 틀린 값을 자신있게 보여주는
    것보다 안전.
    """
    try:
        with conn.cursor() as cur:
            ...
            if dist_km is not None and dist_km > KNN_CONFIDENT_KM:
                return None  # 500m 넘게 떨어진 KNN 매칭은 "농지 아님"으로 취급
