  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = mode === 'login'
        ...
        : await api.register({ email: form.email, password: form.password, name: form.name, phone: form.phone || undefined, role })
  ...
  return (
    <div className="page-wrap" style={{ maxWidth: 480 }}>
      ...
      <form className="card" onSubmit={handleSubmit}>
        ...
        {mode === 'register' && (
          <div className="form-group">
            <label>연락처 {role === 'YOUNG' ? '(상담 신청 시 농장주에게 전달)' : '(선택)'}</label>
